...
func _draw() -> void:
	...
	if not selected.is_empty():
		...
		for x in range(-4, 5):
			for y in range(-4, 5):
				...
				if model.placement_error(cell_to_world(cell), selected).is_empty():
	...
	if model.modules.has(inspected_position):
		draw_rect(Rect2(world_to_screen(inspected_position) - Vector2.ONE * 28, Vector2.ONE * 56), Color("88d9c6"), false, 1.5)
	if not selected.is_empty() and absi(hover_cell.x) <= 4 and absi(hover_cell.y) <= 4:
		...
		var rect := Rect2(world_to_screen(placement_position(get_global_mouse_position())) - Vector2.ONE * (cell_size / 2 - 2), Vector2.ONE * (cell_size - 4))
...
func placement_position(screen_position: Vector2) -> Vector2:
	...
	return (world_position / snap_spacing + Vector2.ONE * 0.5).floor() * snap_spacing
