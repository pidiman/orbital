...
static func validate_graph(data: Dictionary, model: StationModel, region_data: Dictionary, transport: Dictionary) -> String:
	...
	for id: Variant in data.structures:
		...
		if record.state.has("refinery_running"):
			if not model.catalog.get(record.kind, {}).has("conversion") or not record.state.refinery_running is bool: return "Invalid refinery running state."
	...
	for id: Variant in data.ships:
		...
		var ship_tier: int = int(ship.get("tier", 1))
		var max_ship_tier: int = model.ship_catalog.get(str(ship.get("kind", "")), {}).get("upgrades", []).size() + 1
		if not integer(ship_tier, 1) or ship_tier > max_ship_tier:
