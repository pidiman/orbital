...
static func draw_module(canvas: CanvasItem, center: Vector2, kind: String, scale_factor: float = 1.0, opacity: float = 1.0, facing: float = 0.0, tier: int = 1, connector_mask: int = 0, module_mask: int = 0) -> void:
	...
	if kind == "ship_dock":
		...
		# Silo base; ThreatField overlays the rotating three-tube launcher.
		...
		canvas.draw_circle(Vector2.ZERO, 14, Color("263b4a", opacity))
		canvas.draw_circle(Vector2.ZERO, 14, red, false, 1.8, true)
