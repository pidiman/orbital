...
The bottom footer uses Variant C: 30 logical pixels high, 14 pixels above the viewport bottom. A compact 250px-wide rounded panel (68% opacity, no border) wraps only the left controls; status text has no background. Individual buttons have no tiles. The visibility toggle hides both controls and their panel, leaving status text visible. Labels have a 3px dark outline; status and zoom labels also have a subtle 1px shadow. Buttons retain 48px minimum width and cyan hover/focus text. Equal side reservations keep messages centered; long messages ellipsize within their own area (full text available as a tooltip), preventing control overlap. The existing visibility preference and all view commands are unchanged. `tests/transparent_footer_playthrough.gd` checks size, transparency, outlines, centering, no overlap, toggle, zoom and unchanged game state at desktop and 960×720 layouts.
