...
Edit `data/station_grid.json` and restart the game. Defaults are `columns: 33`
and `rows: 33`: 1,089 cells, increased from 17×17 / 289. Each dimension must be an odd
...
default cell centers range from −928 to +928 on both axes. Continuous positions,
...
bounds and loads it under 33×33. It verifies exact positions and identities,
...
Floating-resource changes are documented separately in `docs/floating-resources.md`; the grid itself adds no save fields.
