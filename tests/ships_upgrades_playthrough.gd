...
func _ready() -> void:
	...
	checks["upgrade_cost_visible"] = game.hud.upgrade_detail.text.contains("30 Materials + 8 Minerals") and not game.hud.upgrade_button.disabled
	await settle(2)
	...
	checks["tier_and_next_shown"] = game.hud.upgrade_title.text.contains("Tier 2") and game.hud.upgrade_button.text.contains("Upgrade to T3") and game.hud.upgrade_detail.text.begins_with("Next:")
