...
func _ready() -> void:
	...
	checks["tile_focuses_physical_home"] = game.fleet.regions.current_region == "home"
	await use(nav.region_buttons.venus)
	await click(game.get_viewport().get_canvas_transform() * (game.asteroids.rocks[target].point))
	checks["regional_click_requires_local_miner"] = not game.fleet.jobs.has(miner) and game.hud.status_label.text.contains("No idle Miner is in this region")
