...
func _ready() -> void:
	...
	checks.parallel_output = m.materials == 56 and m.minerals == 8 and game.fleet.diplomacy.inventory.tech == 1
	checks.progress = m.refinery_progress[ore] == 2 and m.refinery_progress[tech] == 0
	...
	game.fleet.diplomacy.inventory.tech = m.capacity
	var minerals: int = m.minerals
	var progress: Dictionary = m.refinery_progress.duplicate()
	for tick in range(30): m.tick()
	checks.full_pauses_both = m.materials == m.capacity and m.minerals == minerals and game.fleet.diplomacy.inventory.tech == m.capacity and m.refinery_progress == progress
	game.fleet.diplomacy.inventory.tech -= 1
	for tick in range(20): m.tick()
	checks.tech_resumes = m.materials == m.capacity - 20 and game.fleet.diplomacy.inventory.tech == m.capacity
	m.tick()
	checks.ore_resumes = m.materials == m.capacity - 14 and m.minerals == minerals - 2
	game.hud.inspect_module(tech)
	await settle(2)
